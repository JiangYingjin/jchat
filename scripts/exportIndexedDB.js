...
async function exportIndexedDB(dbName) {
    return new Promise((resolve, reject) => {
        ...
        request.onerror = (event) => {
            console.error(`Error opening database ${dbName}:`, event.target.errorCode);
        ...
        request.onsuccess = (event) => {
            ...
            for (const storeName of db.objectStoreNames) {
                ...
                cursorRequest.onerror = (e) => {
                    console.error(`Error opening cursor for store ${storeName}:`, e.target.errorCode);
                    reject(`Error opening cursor for store ${storeName}: ${e.target.errorCode}`);
                    db.close();
                };
            }
            ...
            transaction.onerror = (event) => {
                console.error(`Transaction error for database ${dbName}:`, event.target.errorCode);
                reject(`Transaction error for database ${dbName}: ${event.target.errorCode}`);
                db.close();
            };
        };
    });
...
exportIndexedDB(JCHAT_DB_NAME)
    ...
    .catch(error => {
        ...
        alert(`导出失败：${error}。请检查控制台以获取更多信息，并确认数据库名称和上下文是否正确。`);
    });
