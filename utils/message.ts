...
export function findMessagePairForResend(messages: ChatMessage[], targetMessageId: string) {
    const resendingIndex = messages.findIndex((m) => m.id === targetMessageId);
    if (resendingIndex < 0) return { userMessage: undefined, botMessage: undefined, requestIndex: -1 };
